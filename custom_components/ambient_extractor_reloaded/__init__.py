"""Module for ambient_extractor_reloaded (RGB extraction from images) component."""
...
SERVICE_SCHEMA = vol.All(
    ...
    cv.make_entity_service_schema(
        {
            **LIGHT_TURN_ON_SCHEMA,
            vol.Exclusive(ATTR_PATH, "ambient_extractor_reloaded"): cv.isfile,
            vol.Exclusive(ATTR_URL, "ambient_extractor_reloaded"): cv.url,
        }
    ),
)
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    async def async_handle_service(service_call: ServiceCall) -> None:
        """Decide which ambient_extractor_reloaded method to call based on service."""
    ...
    async def async_extract_color_from_url(url):
        ...
        with io.BytesIO(content) as _file:
            _file.name = "ambient_extractor_reloaded.jpg"
            _file.seek(0)
